...
class RarRecoveryApp:
    def __init__(self, root):
        ...
        self.root.geometry("520x460")  # Expanded height slightly for length inputs
        ...
        self.min_len = tk.StringVar(value="1")
        self.max_len = tk.StringVar(value="4")
    ...
    def create_widgets(self):
        ...
        ttk.Radiobutton(mode_frame, text="Numeric Brute Force (1, 2, 3...)", variable=self.attack_mode, 
                        value="numeric", command=self.update_ui_state).pack(anchor="w", pady=2)
        ttk.Radiobutton(mode_frame, text="Dictionary Attack (Uses Wordlist)", variable=self.attack_mode, 
                        value="dict", command=self.update_ui_state).pack(anchor="w", pady=2)
        ttk.Radiobutton(mode_frame, text="Alphanumeric Brute Force (a-z, A-Z, 0-9, !@#)", variable=self.attack_mode, 
                        value="alpha", command=self.update_ui_state).pack(anchor="w", pady=2)

        # --- Dynamic Length Settings (For Alphanumeric) ---
        self.length_frame = ttk.Frame(mode_frame, padding=5)
        ttk.Label(self.length_frame, text="Min Length:").grid(row=0, column=0, padx=5)
        self.min_entry = ttk.Entry(self.length_frame, textvariable=self.min_len, width=5, state="disabled")
        self.min_entry.grid(row=0, column=1, padx=5)
        
        ttk.Label(self.length_frame, text="Max Length:").grid(row=0, column=2, padx=5)
        self.max_entry = ttk.Entry(self.length_frame, textvariable=self.max_len, width=5, state="disabled")
    ...
    def update_ui_state(self):
        """Manages which input fields are active based on selected attack mode."""
        mode = self.attack_mode.get()
        
        # Wordlist fields toggling
        if mode == "dict":
        ...
        # Alphanumeric length fields toggling
        if mode == "alpha":
            self.length_frame.pack(anchor="w", pady=5, padx=20)
    ...
    def start_process(self):
        ...
        # Length validation for Alphanumeric mode
        if self.attack_mode.get() == "alpha":
            try:
                minimum = int(self.min_len.get())
                maximum = int(self.max_len.get())
                if minimum <= 0 or maximum <= 0 or minimum > maximum:
                    raise ValueError
            except ValueError:
                messagebox.showwarning("Invalid Input", "Please enter valid numbers for Min and Max lengths (Min must be <= Max).")
                return
